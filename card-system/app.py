...
while True:
    try:
        db = mysql.connector.connect(
            ...
            database=os.getenv("DB_NAME", "card_system")
        )
...
def ensure_threshold_row():
    ...
    if cursor.fetchone()[0] == 0:
        cursor.execute(
            "INSERT INTO failure_logs (id, blur_score, brightness_std, mean_intensity, file_size, name_mismatch, reason) VALUES (%s,%s,%s,%s,%s,%s,%s)",
...
@app.post("/extract")
async def extract_card(file: UploadFile = File(...), name: str = Form(...)):
    ...
    if name.upper() not in raw_text.upper():
        cursor = db.cursor()
        user_id = generate_unique_code("F")
        cursor.execute(
            "INSERT INTO failure_logs (id, reason, name_mismatch) VALUES (%s, %s, %s)",
            (user_id, "name mismatch", 'Y')
        )
        db.commit()
        raise HTTPException(status_code=400, detail="Provided name not found in document")
